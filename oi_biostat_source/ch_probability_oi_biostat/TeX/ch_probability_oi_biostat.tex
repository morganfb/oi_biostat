...
\textit{JV: This section comes across as rather lifeless right now. Could add an example in which independence is illustrated using the conditional probability formula.}
...
{[P(\text{mammogram$^+$ } | \text{ no BC}) \times P(\text{no BC})] + [P(\text{mammogram$^+$ } | \text{ has BC}) \times P(\text{has BC})]}
\end{align*}
...
\textit{JV: I'm not as much a fan of that figure. I think the physical interpretation worked better for the initial introduction of a mean, but then here, the idea of an "expected value" is already a clear interpretation.}
...
The coin tossing scenario provides a simple illustration of the mean and variance of a random variable, but it is artificial.  A more realistic example follows.
...
There are a few special cases of this formula which are especially useful. The formula implies that $E(a + Y = a + E(Y)$.  This could have been used when calculating the average health costs for the employee by defining $a$ as the fixed cost of the premium ($a=\$948$), and taking $Y$ to be the cost of the physician visits. Thus, the total annual cost for a year in which there were two physician visits could be calculated as: $E(a + Y = a + E(Y) = \$948 + E(2) = \$948 + \$40 = \$988$. 
...
\textit{JV: For the following exercise, are you asking for expected cost taking only her daughter's visits into account? Or incorporating the previously given information about the employee's visits?}

\begin{exercise}
	Suppose the employee is considering adding her 23-year-old daughter to her health plan. She will have to switch to a family plan, with a monthly premium of \$213, and has agreed to cover the costs of the premium and physician visits for her daughter.  In the last five years, her daughter visited a physician only once for three years, and twice in the other two years.  Assuming that the cost of physician visits under the family plan is the same as under the individual plan, calculate the expected cost to the employee in the next year.\footnote{Solution needs to be added.}
\end{exercise}
...
To calculate the variance and standard deviation for five years of health care costs (under the individual plan):
...
The standard deviation is $\sqrt{(25)(\$1556)} = \$197.23$.

The example of health insurance cost has been simplified to make the calculations clearer.  It ignores the fact that many plans have a deductible amount, and that plan members pay for services at different rates before and after the deductible has been reached. Often, insured individuals do not pay for services at all once a maximum amount has been reached in a year.  The exercises contain more realistic examples. 
...
\begin{exercise}
Three US adults are randomly selected. The probability a single adult is between \resp{180} and \resp{185} cm is 0.1157.\footnote{(a) Assume independence. $0.1157 \times 0.1157 \times 0.1157 = 0.0015$. (b) $(1-0.1157)^3 = 0.692$} \vspace{-1.5mm}
\begin{enumerate}
\setlength{\itemsep}{0mm}
