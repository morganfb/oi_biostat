...
introduced a method for finding the variance and standard deviation for a data set.  Deviations from the mean ($x_i - \mu$) were squared, then averaged to compute the variance. In the case of a random variable, squared deviations are again used, this time deviations from the mean of the random variable, and their sum is weighted by the corresponding probabilities. This weighted sum of squared deviations equals the variance, and the standard deviation is again the square root of the variance, just as in Section~\ref{variability}.
...
This compact form is not useful for direct computation, but it useful when understanding the general concept. 
...
Sums of random variables arise naturally in many problems.  In the example of the cost of  health care under an insurance plan, the amount spent by the employee during her next five years of employment can be represented as $X_1 + X_2 + X_3 + X_4 + X_5$, where $X_1$ is the cost of the first year, $X_2$ the second year, etc. If the employee's domestic partner also has health insurance with another employer, If the employee decides to add change to a family plan and add her 23 year old daughter to the plan, the total annual cost to the couple would be the sum of the costs for the employee ($X$) and her partner ($Y$), or $X + Y$.  In each of these examples, it is intuitively clear that the average cost would be the sum of the average of each term.

Sums of random variables are special cases of linear combinations of variables.  

\begin{termBox}{\tBoxTitle{Linear combinations of random variables and their expectation}
...
There are a few special cases of this formula which are especially useful.  The formula implies that $E(a + Y) = a + E(Y)$.  This could have been used when calculating the average health costs for the employee by letting $ = \$948$m the fixed cost of the premium, and taking $Y$ to be the cost of the physician or physical therapy visits. The formula easily generalizes to a sum of any number of random variables, the average health care cost for 5 years would be 

\begin{align*}
	E(X_1 + X_2 + X_3 + X_4 + X_5) &= E(X_1) + E(X_2) + E(X_3) + E(X_4) + E(X_5) \\
	         &= 5E(X_1),
\end{align*}
as long as the cost for sevices under the plan does not change for 5 years.

\begin{exercise}
	Suppose the employee is considering adding her 23 year old daughter to her health plan.  She will have to switch to a family plan, with a monthly premium of \$213, and has agreed to cover the costs of the premium and physician visits for her daughter.  The last 10 years of health care use for the daughter includes the period 13 - 18 years of age, when she was a high school field hockey player and injury prone, so she reviews only the last five years.  During that period, her daughter visited a physician  only once for three years, and twice in the other two years.  Assuming that the cost of physician visits under the family plan is the same as under the individual plan, calculate the expected cost to the employee in the next year.
\end{exercise}

The employee might like to know how variable her health care costs would be for the next five years under the individual plan. Calculating the variance and standard deviation of a linear combination of random variables requires more care.  The formula given here requires that the random variables in the linear combination be independent, so that an observation on one of the variables provides no information about the value of the other variable.  With that assumption, the following formula can be used:
...
\begin{align*}
\text{Var}(aX + bY) = a^2\times \text{Var}(X) + b^2\text{Var}(Y)
\end{align*}
...
To calculate the variance and standard deviation for 5 years of health care costs.

\begin{align*}
	\text{Var}(X_1 + X_2 + X_3 + X_4 + X_5) &= \text{Var}(X_1) + \text{Var}(X_2) +\text{Var}(X_3) +\text{Var}(X_4) +\text{Var}(X_5) \\
	&= 5\text{Var}(X_1) \\
	& = (5)(39.45) = 197.25	
\end{align*}

The standard deviation is $\sqrt{(5)(39.45)} = (\sqrt{5})(\sqrt{39.45}) = \$14.45$.  So the standard deviation of the sum of the five terms is $(\sqrt{5})\sigma_{X_1}$.

The example of health insurance cost has been simplified to make the calculations clear.  It ignores the fact that may plans have a deductible amount, and that plan members pay for services a different rates before and after the deductible has been reached. Often, insured individuals do not pay for services at all once a maximum amount has been reached in a year.  The exercises contain more realistic examples. 

\begin{exercise}  The Association of American Medical Colleges (AAMC) introduced a new version of the Medical College Admission Test (MCAT) in the spring of 2015 and data from the scores were recently relased by AAMC\footnote{https://www.aamc.org/students/download/434504/data/percentilenewmcat.pdf}.  The test consists of 4 components: chemical and physical foundations of biological system; critical analysis and reasoning skills; biological and biochemical foundations of living systems; and psychological, social and biological foundations of behavior. The overall score is the sum of the individual component scores. The grading for each of the 4 components is scaled so that the mean score is 125.  The means and standard deviations for the 4 components and the total score for the population taking the exam in May 2015 exam are shown in table~\ref{mcatScoreDistribution}.
	\begin{table}[h]
...
\textit{The second part of this will be difficult for students, esp without the notion of correlation of random variables. The simple answer is that the component scores are not independent.  More precisely,  the component scores will be positively correlated, increasing the variance of the sum.  Students who score high (low) on one component tend to score high (low) on all. It is an important concept, but difficult to cover with just intuition.  It may be a reason to introduce general correlation. }

\textit{Because of this example, I think we should reconsider adding correlation to chapter 1, so that we can introduce joint distributions in chapter 2.  We have plenty of examples in chapter 1 where correlation is releveant}
...
Adding more bins provides greater detail. This sample is extremely large, which is why much smaller bins still work well.  So many bins are generally not used with smaller sample sizes since small counts per bin make the bin heights are volatile.
\end{example}

\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{ch_probability_oi_biostat/figures/fdicHistograms/fdicHistograms}
...
First add the heights of the bins in the range \resp{180} cm and \resp{185} and divide by the sample size. For instance, this can be done with the two shaded bins shown in Figure~\ref{usHeightsHist180185}. The two bins in this region have counts of 195,307 and 156,239 people, resulting in the following estimate of the probability:
\begin{eqnarray*}
...
\begin{figure}
\centering
\includegraphics[width=0.9\textwidth]{ch_probability_oi_biostat/figures/usHeightsHist180185/usHeightsHist180185}
...
\begin{figure}[tbh]
\centering
\includegraphics[width=0.9\textwidth]{ch_probability_oi_biostat/figures/fdicHeightContDist/fdicHeightContDist}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{ch_probability_oi_biostat/figures/fdicHeightContDistFilled/fdicHeightContDistFilled}
