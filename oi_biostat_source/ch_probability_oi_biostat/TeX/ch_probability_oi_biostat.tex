...
\begin{table}[ht]
	...
	\begin{tabular}{rrrr}
		...
		\hline
	\end{tabular}
	\caption{Completed table for the mammogram example. The table shows why the PPV of the mammogram is low: almost 7,300 women will have a positive mammogram result in this hypothetical population, but only \textasciitilde312 of those women actually have breast cancer.}
...
There are a few special cases of this formula which are especially useful. The formula implies that $E(a + Y) = a + E(Y)$.  This could have been used when calculating the average health costs for the employee by defining $a$ as the fixed cost of the premium ($a=\$948$) and $Y$ as the cost of the physician visits. Thus, the total annual cost for a year could be calculated as: $E(a + Y) = a + E(Y) = \$948 + E(Y) = \$948 + .30(1 \times \$20) + .40(3 \times \$20) + .20(4 \times \$20) + .10(8 \times \$20)= \$1,010.00$. 
...
\textit{Formal solution to be added. Start with X as the costs for employee and Y as the costs for the partner, use formula for Var(X + Y). Var(X) is \$1,556.00 from previous example; E(Y) is \$980, and Var(Y) is \$268.80. Thus, Var(X + Y) = \$1,824.80. However, is the independence assumption reasonable here?}

%JV: Needs explanation for independence assumption.
...
\textit{Solution to be added in later draft \dots}

%DH: Solution to be added, but only as far as noting that independence is not likely to be true. Begin by applying the formula that assumes independence.
...
\textit{Section to be finished in subsequent draft \dots}
