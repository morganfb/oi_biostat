...
\textit{JV: Perhaps it is not obvious that the outcomes are equally likely if one is both new to genetics and probability (without calculating that each is indeed 1/4)?}
...
\textit{JV: note to self, fix color of dice (and also in example...)}
...
\begin{figure}[hht]
\centering
\includegraphics[width=0.6\textwidth]{ch_probability_oi_biostat/figures/indepForRollingTwo1s/indepForRollingTwo1s.png}
...
Since the problem assumes that affected parents do not reproduce, the pair of parents must satisfy one of the events $A = \text{(neither parent is a carrier)}$, $B = \text{(the mother is a carrier, the father is not)}$, $C = \text{(the father is a carrier, the mother is not)}$, and $D = \text{(both parents are carriers)}$.  Since the carrier status of the mother is independent of that of the father, the probabilities of these events are
...
\textit{solution continues now with the conditional probabilities and the tree.  We should think about whether this example is too big of a step from the others, and perhaps add an intermediate example}
...
The problem arises so often in medicine, and is so important, that is useful to have several ways to approach the problem -- tree diagrams, a purely algebraic approach using Bayes' Theorem, and the construction of a large, hypothetical population that allows conditional probabilities to be calculated from contingency tables.  The first solution uses tree diagrams and more specific information about mammograms and breast cancer.
...
\end{align*}
Even with a positive mammogram, there is still only a~4\%~chance of breast cancer.  Most people find this a surprising result.  \textit{return to this to finish why low prevalence leads to low ppv. that is the important concept for pre-meds and others}
\end{example}
