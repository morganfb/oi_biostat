...
%\textit{Agree, but we don't have good examples yet.}
...
%  This needs an example.  Using the diabetes example would seem circular, since it was used to define/illustrate cond prob.
...
It is not difficult to show in Guided Practice~\ref{condProbOfRollingA1AfterOne1}(c) that the conditioning information has no influence by using the Multiplication Rule for independent events:
\begin{eqnarray*}
...
%JV: In my edit of Solution 3, I removed the references to specificity and sensitivity, since the problem is only introduced in the context of false positives and false negatives. For a later revision, we could introduce language/notation specific to diagnostic tests at the end of this section.

%  These refs to spec and sensitivity can be re-inserted
...
For the simple transformation $a + bZ$, the variance is $b^Z$, since a constant $a$ has variance 0.  When $b = 1$, variance of $a + Z$ is $\text{Var}(Z)$ -- adding a constant to a random variable has no effect on its variability.
...
\begin{example}{Calculate the standard deviation of the sum of the costs for the couple.}
