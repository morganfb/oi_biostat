...
\eocesol{\ \\
\includegraphics[width = 40mm]{ch_intro_to_data_oi_biostat/figures/eoce/reproducing_bacteria/reproducing_bacteria_sketch.pdf}}
...
(b)~Proportion. Each respondent responds Yes or No, so this is a categorical 
...
(d)~No. There exists sampling variation among sample means. The sample that the original researchers observed is likely to be different than the sample of 129 frogs. These point estimates both approximate the population parameter, but values vary from one sample to another.  
...
variability with the standard error. $SE = 18.2 / \sqrt{45} = 2.71$.
(d)Even though the distribution of ages in the sample is skewed to the right, the number of observations in the sample is significantly larger than 30. Assumptions of independence have not yet been verified, but the sampling distribution can still be approximated to a normal distribution regardless of skew. 
...
(d) To guarantee that the population parameter is captured, the interval needs to be $(0, 24)$. This does not provide any insight to the actual number of hours that U.S. residents have to enjoy. \emph{Note:} An answer of $(-\infty, \infty)$ is nonsensical. Always remember the context of the question when preforming inference.
}
...
(d) Without changing the confidence level, the width can be modified though its standard error (standard deviation and number of observations). The number of observations can increase so that the standard error is smaller resulting in a smaller confidence interval width.
}
