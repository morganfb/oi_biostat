...
These questions encompass the broader idea of statistical inference covered in Chapter \ref{foundationsForInference}. Inference is a set of tools used to estimate properties about a population, also known as parameters, after observing a sample. Inference allows for different levels of confidence for these property estimates. Once the estimate for the average adult BMI in the United States is calculated, scientists can ask how confident are they that this estimate is representative of the greater adult population in the US. For example, a classic inferential question is, ``How likely is it that the estimated mean, $\bar{x}$, is near the population mean, $\mu$?'' Statistical inference includes asking these questions but also determining which estimates to use.
...
Section \ref{brfssData} introduces a potential dataset that the CDC would use to answer the above questions. Section \ref{variabilityInEstimates} provides an introduction to point estimates, the distribution that these estimates are drawn from, and the variability within these estimates. Section \ref{confidenceIntervals} presents the concept of a confidence interval -- a range of values that incorporate variability. Finally Section \ref{hypothesisTesting} introduces hypothesis testing, allowing researchers to compare an observed point estimate to a benchmark. The hypothesis testing framework provides structure to encompass variability and uses the same moving parts as the calculation for a confidence interval. The subsequent sections \ref{tdistribution} - \ref{sampleSizeAndPower} provide additional insight into general inference tools.
...
A likely choice to estimate the \term{population mean} from the \data{BRFSS BMI} sample is to take the \term{sample mean}. That is, use the average BMI of all 40 survey respondents in the sample as the estimate for the average BMI among US adults. 
...
\begin{eqnarray*}
\bar{\mathrm{bmi}} = \frac{30.41 + 23.40 + 25.10 + \dots}{40} = 26.53
\end{eqnarray*}
\index{point estimate!single mean|(}
and is the \term{point estimate} of the population mean\footnote{If \var{weight} is the variable of interest instead, the sample mean of observations denoted $w_1,\ldots w_{40}$ would be $\bar{w}$}. 
...
The CDC was interested in an estimate for difference in the average age for adult men and women in the US. If $\bar{\mathrm{age}}_{\mathrm{women}} = 50.22 $ years and $\bar{\mathrm{age}} _ {\mathrm{men}} = 47 $ years, what would be a good point estimate for the population age difference? \footnote{Take the difference of the two sample means: $\bar{\mathrm{age}}_{\mathrm{women}} - \bar{\mathrm{age}} _ {\mathrm{men}} = 50.22 - 47 =  3.22$. Women are on average estimated to be older than men by 3.22 years.}
\end{exercise}
...
Suppose from the original respondents in \data{BRFSS}, a different sample of 40 people is observed and the mean calculated; the answer would likely not be the same as $26.53$, the sample mean from \data{BRFSS BMI}. Estimates generally vary from one sample to another even with samples of the same size. Low \term{sampling variation} can suggest that the estimate may be close, but not exactly equal to the parameter. A larger sample size can ensure a closer estimate to the population parameter than an estimate taken from a smaller sized sample. 

Using $R$, take another simple random sample from the \data{BRFSS} data of 40. The new sample mean for the BMI is 28.17. Doing this again, the average BMI is 24.89. Estimates differ across samples through sampling variation but the accuracy of the point estimate will improve once sample sizes increase. In practice, it is extremely rare to observe more than one sample from a population.

A \term{running mean} - a sequence of means where each mean uses one more observation in its calculation than the mean directly before it in the sequence - demonstrates sampling variation and increasing accuracy for larger sample sizes. For \data{BRFSS BMI}, the second mean is the average of the first two observations, $\mathrm{bmi}_1, \mathrm{bmi}_2$. The third number in the running mean sequence is the average of $\mathrm{bmi}_1, \mathrm{bmi}_2,$ and $\mathrm{bmi}_3$. 
...
\begin{figure}
   ...
   \caption{The running mean from the \var{BRFSS BMI} sample of 40 observations. The mean stabilizes and approaches $\bar{\mathrm{bmi}} = 26.53$ as the number of observations increases to 40.}
      \label{BMIRunningMean}
\end{figure}

Sampling variation is across samples of the same size. Figure ~\ref{runningSamplingVariation} displays the running means of two samples of size 40 from \data{BRFSS}. With a small sample size, the path of the running means are not the same. There exists a substantial amount of sampling variation. The variation across the different running means decreases as the number of observations increases. 

Figure ~\ref{runningSamplingVariation20} illustrates this more clearly by calculating the running means of 20 independent samples of 40 observations. The sampling variation is large with small sample sizes $n$, but as the number of observations get larger, the sampling variation decreases. This concept will be explored more in Section ~\ref{accuracyAndPrecision}. 
...
\begin{figure}
   ...
   \caption{Many running means from different samples of 40 observations from \data{BRFSS}. The sampling variation decreases as the number of observations gets larger.}
...
\begin{figure}
   ...
   \caption{A histogram of 100,000 sample means for BMI, where the samples are of size $n=40$. }
      \label{brfssBMISamplingDistribution}
\end{figure}

Figure~\ref{brfssBMISamplingDistribution} is an approximation of the sampling distribution. The approximation resembles the exact sampling distribution as more and more samples are drawn. To get the exact sampling distribution, the CDC would need to sample every possible unique combination of 40 respondents from the entire US adult population (and not from the \data{BRFSS} dataset).  The CDC would never create a sampling distribution and instead use all 170,000 observations to calculate one sample mean.\footnote{In practice, researchers are almost never capable of taking more than one sample from the target population.} However it is extremely important for the CDC to know that the single sample they observe is one observation out of the many possible sample means within the sampling distribution. 

With access to the \data{BRFSS}dataset, an approximation of the sampling distribution of the sampling mean can be created with the following pseudocode \footnote{Refer to the appendix for R code}: 
...
The sampling distribution for the sample mean is unimodal and symmetric, centered at the target population's mean. Intuitively, this makes sense. The sample means should tend to ``fall around'' the mean of the target population. From the sampling distribution, it's easy to understand why the sample mean is the best guess for the population mean.

A point estimator will have some variability. Figure ~\ref{brfssBMISamplingDistribution} demonstrates that the estimator for the average adult BMI can range from 24 to 29. This range is specific to sample of size 40. Section ~\ref{accuracyAndPrecision} introduced that variability will decrease as the sample size gets larger. There must exist some metric to quantify the variability of the point estimate around the parameter that is sample size specific. 
...
Standard deviation is the most obvious method to quantify variability. The standard deviation of the sampling distribution denoted  $\sigma_{\bar{x}}$ or $s$ measures the sampling variation of the sample mean. \footnote{Caution: The standard deviation of the sample mean is not equivalent to the estimate for the standard deviation of the population. Those are measuring two separate quantities.}

The standard deviation of the sample mean indicates how far the typical estimate is away from the population mean. It is a very good metric to size the typical \term{error} of the point estimate, and for this reason, an estimate's standard deviation is called the \term{standard error (SE)} \index{SE}\marginpar[\raggedright\vspace{-4mm} $SE$\\\footnotesize standard\\error]{\raggedright\vspace{-4mm} $SE$\\\footnotesize standard\\error} of the estimate. 
...
Remember, the term "standard error" is not interchangeable with "standard deviation". Standard deviation describes the spread of values within one sample. The standard error of the sample mean describes how accurate the sample mean is to the population mean. It represents the spread among all sample means.}
\end{tipBox}
...
The definition of standard error suggests that it is the measure of spread among all the sample means -- the standard deviation of the sampling distribution. Calculating the exact sampling distribution is very uncommon. With the ability to repeatedly sampling, the standard deviation of the approximation of the sampling distribution can be used as a substitute. Using pseudocode from earlier in the section, an standard error estimate is the standard deviation of all the sample means. The algorithm can be implemented in $R$:\footnote{This computing experience samples without replacement to simulate experimenting in the real world (researchers would never ask the same person twice). Theory states however that individual BMI values need to be independent. A reliable method to ensure independence is to use the 10\% rule of thumb: simple random sample with a size less than 10\% of the population. Sampling without replacement within a finite population results in a reasonable estimate for the standard error}
...
After 10,000 iterations the estimated standard error \data{sd(sample.means)} is $0.83$. This method, however, has one problem: most often, scientists only observe one sample $\bar{x}$. They don't have the chance to repeatedly sample and there is no obvious way to estimate the standard error from a single sample. What do they do? Instead the standard error of the sample mean can be calculated through the following equation: 
...
There is one subtle issue of Equation~(\ref{seOfXBar}): the population standard deviation is typically unknown. Section \ref{pointEstimates} suggests that the "best guess" for the population standard deviation can be used as a substitute. This estimate tends to be sufficiently good when the sample size is at least 30 and the population distribution is not strongly skewed, so practitioners then replace $s$ for $\sigma$ in Equation ~\ref{seOfXBar}. When the sample size is smaller than 30 or the skew condition is not met, a larger sample or other methods need to be used. These topics are further discussed in Section~\ref{cltSection}. 

The standard error of the sample mean calculated from observing \data{BRFSS BMI} is 

\begin{eqnarray*}
SE_{\bar{x}} = \frac{s}{\sqrt{n}} = \frac{5.84}{\sqrt{40}} =  0.92
\end{eqnarray*}
where $s$ is the standard deviation of the sample and $n$ is the number of observations in the sample. We see that the standard error calculated $(0.92)$ is similar to the empirical standard deviation of the sampling distribution that was calculated above $(0.83)$ \footnote{For context, the sample taken in Section ~\ref{pointEstimates} with mean 28.17 results in a standard error of $8.85/\sqrt{40} =1.40$}. 

\begin{exercise}
In another sample of 40 US adults, the standard deviation of BMI is $s_\mathrm{bmi} = 4.81$. The sample is a simple random sample of less than 10\% of the United States population. The observations are independent. (a)~What is the standard error of the sample mean, $\bar{bmi}=24.89$? (b)~Is it surprising if the CDC published a report with the average BMI of all US adults as 30? How about 25?

\footnote{(a) Use Equation~(\ref{seOfXBar}) with the sample standard deviation to compute the standard error: $SE_{\bar{\mathrm{bmi}}} = 4.81/\sqrt{40} =  0.76$. (b) It is surprising if the average adult BMI of the US population was 30. A BMI of 30 is many many standard deviations away (0.76) from the sample mean of 26.36. 
...
This section achieved three goals. First, point estimates from a sample are used to estimate population parameters. Furthermore, these point estimates are not exact, and there exists some sampling variation. The sample mean is an example of a point estimate that is always accurate but not necessarily always precise. The precision of a point estimate can be represented through sampling variation and visualized through many running means or a sampling distribution. The sampling distribution, a theoretical and fundamental concept, represents all the possible sample estimates. A point estimate from an observed sample is only a single value within the estimate's sampling distribution. Lastly, sampling variation is measured during the standard error. This uncertainty of the sample mean can be mathematically calculated from Equation~\eqref{seOfXBar} or approximately computed in $R$. The standard error and estimates can be quantified for other parameters  -- the median, standard deviation, or any other number of statistics . These extensions will be postponed until later chapters and courses.
...
A point estimate provides a single plausible value for a parameter and is rarely perfect and exact; usually there is some error in the estimate. The standard error conveys how large this sampling variation is but a single point estimate alone does not. Is there a single measurement that conveys a point estimate and the sampling variation in one? 

This section and Section~\ref{hypothesisTesting} focuses on the sample mean as the point estimate and the population mean as the parameter. Section~\ref{aFrameworkForInference} generalizes these methods for a variety of point estimates and population parameters that will be encounter in Chapter~\ref{inferenceForNumericalData} and beyond.

\subsection{Capturing the population parameter}

Using only a point estimate is like fishing in a murky lake with a spear. Using a confidence interval is like fishing in the same lake with a net. Fishermen who fish with spears will probably miss. On the other hand, if fishermen use nets in that area, they are more likely to catch a fish. 

Similarly a point estimate is likely to not equal the exact population parameter. The single point estimate lacks information about the standard error. If a range of plausible values is reported -- \term{a confidence interval} -- it becomes more likely that the parameter is captured. The range encompasses sampling variation. The width of the interval is a metric of how large the sampling variation is. The larger the interval generally indicates the large the sampling variation and standard error \footnote{It could also indicate the higher confidence level. This will be introduced later}. As with fishing, the goal of the confidence interval is to include the population parameter within it. 
