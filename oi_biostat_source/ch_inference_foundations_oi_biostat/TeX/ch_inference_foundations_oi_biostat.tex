...
The United States Center for Disease Control and Prevention (CDC) focuses "on policy and environmental strategies to make healthy eating and active living accessible and affordable for everyone." \footnote{\url{http://www.cdc.gov/obesity/}}. One of the CDC's main priorities is understanding national obesity. A person's Body Mass Index, also known as BMI, captures both a person's height and weight within one measurement --a high BMI is an indicator for high body fate. In medicine, BMI categorizes a person as "underweight," "overweight" or "obese." 

These policy makers are interested in a couple of questions. What is the average population BMI for all adults in the United States? Instead of providing a single number, can the CDC provide a reasonable range for the average adult BMI in the United States? Finally using the range of BMI values that the World Health Organization (WHO) provides, is the average adult BMI in the United States considered healthy? Below is a categorization of BMI values and ranges
...
This simple random sample of 40 from \data{BRFSS}, \data{BRFSS BMI}, will be used to draw conclusions about the target population of US adults. In practice, however, the CDC would simply use all 170,000 in its sample. US adults are not strictly the target population of the \data{BRFSS BMI} sample of 40 since they were not directly sampled from the US adult population but from \data{BRFSS}. However if one believes that the \data{BRFSS} dataset is a good surrogate for the US population \footnote{We do.}, then drawing samples from the 170,000 observations within \data{BRFSS} should be analogous to drawing samples from the entire US adult population. Going forward, drawing from the \data{BRFSS} dataset corresponds to drawing from the US population of adults directly. 
...
A likely choice to estimate the \term{population mean} from the \data{BRFSS BMI} sample is to simply take the \term{sample mean}. That is, use the average BMI of all 40 survey respondents in the sample as the estimate for the average BMI among US adults. 
...
Suppose the CDC was interested in an estimate for the average age for men and women in the US as well as the difference in ages. If $\bar{\mathrm{age}}_{\mathrm{women}} = 50.22 $ years and $\bar{\mathrm{age}} _ {\mathrm{men}} = 47 $ years, what would be a good point estimate for the population age difference? \footnote{Take the difference of the two sample means: $\bar{\mathrm{age}}_{\mathrm{women}} - \bar{\mathrm{age}} _ {\mathrm{men}} = 50.22 - 47 =  3.22$. Women are on average older than men by 3.22 years.}
\end{exercise}
...
The sample mean calculated from this \data{BRFSS BMI} sample of 40 will likely be different from the sample mean of a different set of 40 respondents from \data{BRFSS} data. Using $R$, take another random sample from the \data{BRFSS} data of 40 and note that that the new sample mean for the BMI is 28.17. Estimates will differ across samples through sampling variation but the accuracy of the point estimate will improve once more data becomes available and sample sizes increase. In practice, it is extremely rare to have the opportunity to observe more than one sample from a population.

A \term{running mean} - a sequence of means where each mean uses one more observation in its calculation than the mean directly before it in the sequence - is used to explore an increase in accuracy for \data{BRFSS BMI}. The second mean is the average of the first two observations, $\mathrm{bmi}_1, \mathrm{bmi}_2$. The third number in the running mean sequence is the average of $\mathrm{bmi}_1, \mathrm{bmi}_2,$ and $\mathrm{bmi}_3$. 
...
\begin{figure}
   ...
   \caption{The running mean from the \var{BRFSS BMI} sample of 40 observations. The mean stabilizes and approaches the mean of the entire sample $\bar{\mathrm{bmi}} = 26.53$ as the number of observations increases to 40.}
...
Figure ~\ref{runningSamplingVariation20} illustrates this more clearly by calculating the running means of 20 independent samples of 40 observations. The sample variation is extremely large, but as the number of observations get larger, the sampling variation decreases. This concept will be explored more in Section ~\ref{accuracyAndPrecision}. 
...
\begin{figure}
   ...
   \caption{The running mean of two samples drawn from \data{BRFSS}. The exists high sampling variation at the beginning but as the sample sizes increase, the running mean values being to converge.}
