...
Many scientific investigations can be addressed with a small number of data collection techniques, analytic tools, and fundamental concepts in statistical inference. This chapter provides a brief introduction to the basic principles of these areas that will be encountered later in the book, and illustrates the important role statistics plays in medicine and biology.
...
\textbf{where are the data for this?}
 ...
 Table~\ref{stentStudyResults} summarizes subject-level data in a helpful way, showing patterns in the data.  For instance, the left side of the table shows that 33 patients on the treatment group experienced a stroke within 30 days.
...
Of the 224 patients in the treatment group, 45 experienced a stroke by the end of the first year, or a proportion of  $45/224 = 0.20$.
...
\end{itemize}
These two summary statistics are useful for examining possible differences between the groups, and they show something that surprised and disappointed the study team: an additional 8\% of patients in the treatment group experienced a stroke. This illustrates how important it is to verify scientific hypotheses experimentally whenever possible.  It also raises implicitly an important statistical issue.  Does the  study provide definitive evidence that stents are harmful? Phrased another way, is the 8\% difference between the two groups larger than one would expect by chance variation alone, leading to a rejection of the notion that it was due to chance? 
...
The material on hypothesis testing will provide the statistical tools to examine this issue. In the stent study those methods showed compelling evidence of harm by stents for the types of subjects participating in this clinical trial. 

\textbf{Be careful:} do not generalize the results of this study to all patients and all stents. This study looked at patients with very specific characteristics who volunteered to be a part of this study and who may not be representative of all stroke patients. In addition, there are many types of stents and this study only considered the self-expanding Wingspan stent (Boston Scientific). 
...
It is important to understand the definitions of variables, even for items that seem obvious. Height and weight are measured in inches and pounds, respectively, not in the metric scale more commonly used outside the United States. The definitions of the variables \var{ndrm.ch} and \var{actn3.r577x} are more complicated and are given   in Table~\ref{FAMuSS_subset_Variables} with the other 5 variables.
...
For example, the first row represents a Caucasian female, age 27, 65 inches tall, who weighs 199 lbs, was genotype CC  and increased her strength in her nondominant arm by 40\% after training. 
The data in Table~\ref{FAMuSS_subset_variables} represent a \term{data matrix}, which is a common way to organize data. Each row of a data matrix corresponds to a unique case, and each column corresponds to a variable. A data matrix for the stroke study introduced in Section~\ref{basicExampleOfStentsAndStrokes} is shown in Table~\vref{stentStudyResultsDF}, where the cases were patients and there were three variables recorded for each patient.
...
The dataset \data{county} summarizes 
 information about the 3,143 counties in the United States; the data source is the US Bureau of the Census. This data set includes information about each county: its name, the state where it resides, its population in 2000 and 2010, per capita federal spending, poverty rate, and five additional characteristics. Each county may be viewed as a case, and there are eleven pieces of information recorded for each case. A table with 3,143 rows and 11 columns could hold these data, where each row represents a county and each column represents a particular piece of information.
...
After data have been collected according to carefully specified criteria,  examining summary statistics \index{summary statistic}  may provide some initial insights to the answers to these three questions. Additionally, graphs can be used to visually summarize data and are useful for answering such questions as well.
...
\begin{exercise}
Examine the variables in the \data{FAMuSS} data set, which are described in Table~\vref{{FAMuSS_subset_Variables}}. Create two questions about the relationships between these variables that seem interesting to you.\footnote{Two sample questions: (1)  Do participants appear respond differently to training according to genotype?  (2)  Do male participants appear to respond differently to training than females.}
...
The first step in conducting research is to identify topics or questions that are to be investigated. A clearly articulated research question is essential in identifying what subjects or cases should be studied, what variables are important, and  how they should be measured. It is also important to consider \emph{how} data are collected so that they are reliable and help achieve the research goals.
...
\end{enumerate}
Each of these questions refers to a target \term{population}. In the first question, the target population is all swordfish in the Atlantic ocean, and each fish represents a case. Often times, it is too expensive to collect data for every case in a population. Instead, a sample is taken. A \term{sample} represents a subset of the cases and is often a small fraction of the population. For instance, 60 swordfish (or some other number) in the population might be selected, and this sample data may be used to provide an estimate of the population average and answer the research question.

\begin{exercise} \label{identifyingThePopulationForTwoQuestionsInPopAndSampSubsection}
For the second  question above, identify the target population and what represents an individual case.\footnote{ (\ref{identifyPopulationOfStentStudy}) A person with severe heart disease represents a case. The population includes all people with severe heart disease.}
\end{exercise}
...
\textbf{revisions to here, 2 jul 2015, fix anecdotal data}
