...
Scatterplots provide a visual display of the relationship between two variables. It is usually denoted by $r$ and takes on values between -1 and 1. The term \term{correlation coefficient} is often used for $r$. If, in a scatterplot, the paired values of two variables tend to lie on a line, then

\begin{itemize}

 \item If $r > 0$, the slope will be positive and variables are positively associated

 \item If $r < 0$, the line will have negative slope and the variables are negatively associate

 \item If $r = 0$, or approximately 0, the line will be horizontal and there is no apparent trend or association.

 \item If $r \pm 1$}, the points will lie exactly on a line, with positive slope if $r = 1$ and negative slope if $r = -1$.
...
\begin{termBox}{\tBoxTitle{Correlation}%
		...
		\begin{align*}
          ...
          & =  \frac{\sum^{n}_{i=1}(x_{{\color{black} i}}
          -\overline{x}) (y_{i}-\overline{y})}
         {\sqrt{\sum^{n}_{i=1}(x_{i}-\overline{x})^{2}\sum^{n}_{i=1}(y_{i}-
         \overline{y})^{2}}}}
 		\label{correlationEquation}
 		\end{align*},
        where $(x_1,y_1), (x_2,y_2), \ldots, (x_n, y_n)$ are the $n$ paired values of $x$ and $y$, and $s_x$ and $s_y$ are the sample standard deviations of $x$ and $y$
        variables, respectively.
\end{termBox}

The first version of the equation shows that $r$ is an average of products of terms, where the components are differences between values of$x$ or $y$ and their respective means, but re-scaled by standard deviations. 

Example~\ref{simpleCorExample} shows the calculation of correlation in a simple (albeit, artificial) example in a table similar to the one used for a calculating a standard deviation.

\begin{example}  Suppose the paired values of $x$ and $y$ are $(1,3), (2,4), and (3,0)$.  It is easy to show that $\overline{x} = 2$, $overline{y} = 3$, $sd(x) = 1$, and $sd(y) = 2.66$. 
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{lllllllll}
            \toprule
            $x$ & $y$ & $\overline{x}$ & \overline{y} & \textbf{Header 5} & \textbf{Header 6} & \textbf{Header 7} & \textbf{Header 8} & \textbf{Header 9}\\
            \midrule
...
\section{Data from Molecular Biology}

\textit{To be added.  No difficult analyses in this section, but I think we should introduced gene expression chips, RNA Seq, perhaps one or two others.  }
\end{doublespace}
