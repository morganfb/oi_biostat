...
%DH: \textit{DH: If we keep this example, we need references.}
...
Scatterplots provide a visual display of the relationship between two variables, which is usually denoted by $r$ and takes on values between -1 and 1. \marginpar[\raggedright$r$\\\footnotesize correlation coefficient]{\raggedright$r$\\\footnotesize correlation coefficient} The term \term{correlation coefficient} is often used for $r$. If, in a scatterplot, the paired values of two variables tend to lie on a line, then $r > 0$, the slope of the line will be positive, and the variables are positively associated.
...
Correlation can be calculated even when the data in a scatterplot do not tend to lie on a line. The correlation of the data in Figure~\ref{incomeLifeExpectancy} is $0.63$.
...
The first draft of the sequencing of the human genome was announced in 2001 \footnote{International Human Genome Sequencing Consortium. Initial sequencing and analysis of the human genome. Nature 409, 860-921 (2001)}\footnote{Science 291, 1304-1351 (2001) DOI: 10.1126/science.1058040}.  Information about the possible sequences of chemical base pairs that are present in human DNA quickly moved from being a theoretical advance in biology to its use in the study of human diseases and other phenotypes.  Rapidly evolving technology has provided the tools to identify regions of the genome that act as genes and the association of particular genetic expression with diseases such as breast cancer and leukemia, and with features of many diverse organisms, from the human immunodeficiency virus (HIV) to type of plants.  

The use of data in molecular biology is a large, complex topic that cannot be covered comprehensively in an introductory statistics text. A complete understanding of the use of statistics with molecular biology data requires careful study of genome biology, of advanced computational methods that are used to manipulate, summarize and draw inferences from the large amounts of data that can be measured at the genome level, and of the underlying ideas of measurement technology that is still improving rapidly.  Neglecting this topic altogether, however, would be a serious oversight, so this section provides a brief summary of the important aspects of genome biology and introduces two widely used technologies (microarrays and RNA-sequencing, or RNA-seq) and some of the statistical methods used to understand the association between human genes and phenotypes.  Microarray technology was developed in the 1990's for the study of HIV and is old, at least by the standards of modern biology, but experiments that used microarrays have led to important clinical applications in cancer and other fields that are still used in clinical decision making.  RNA-seq is newer, provides an alternate, more flexible technology for examining the interaction between genes and phenotypes, and can be used in a wider diversity of experiments.  Examples of data from both technologies appear later in this text.
...
\subsection{Microarrays (gene-chips)}

\index{data!Golub|(}
 
Microarrays are based on a simple idea: fabricate a silicon wafer or chip (called a gene chip), then attach short sequences of \textsl{DNA} to a grid on the chip. The sequences are usually 25 base-pairs in length, and are called \textit{probes}. There are usually 40 probes for each gene of interest, with each probe containing a different portion of the gene.  Usually for each gene of interest, one set of probes are perfectly matched (PM probes) to the sequence present in the gene, and one set have a single base in the middle of the sequence that does not match the sequence (mismatch or MM probes). The \textsl{mRNA} from a biological specimen is then converted to complementary-\textsl{DNA} (\textit{cDNA}), which will attach itself (hybridize) to PM probes and should avoid the MM probes. The \textit{cDNA} is placed in a solution used to bath the chip; probes which have a have a higher frequency of bound \textit{cDNA}  to PM vs MM probes are then assumed to represent genes which are more active in the specimen.  The technology of chip fabrication has improved along with the understanding of gene sequences, and the newest chips contain probes for more than 100,000 genotypes. While the specific technology of the measurements is too detailed to discuss here, understanding the basic nature of the measurements is essential for interpreting an experiment.

Using microarrays to infer the expression of genotypes involves several steps.  The \textsl{cDNA} is labeled with a fluorescent dye when it is placed in a solution, and after the probes and \textsl{cDNA} have been hybridized, a light source is used to illuminate the dye. The light signal is quantified using a scanner, which detects the number of photons emitted from the probes.  PM and MM pairs with large differences in the average of the PM - MM quantification are assumed to have detected relatively more \textsl{mRNA} and (by inference) more protein in the cell from that gene compared to other genes. It is important to keep this last step in view: the data from a gene chip do not directly measure the presence of a gene or its protein product. The intensity of the light signals can only be used to examine which of the genes represented on the chip seem to be more active in producing protein products.

The data from a gene chip is messy.  Chips may have imperfections during fabrication, even `perfect chips' may vary from batch to batch in the attachment of the probe sets, genes can bind to the MM probes even when they should not, and the quantification of the light source is subject to error.  Biologists, bioinformaticians, and computational biologists have done considerable research on how to `pre-process' chip data to adjust for these and other errors.  This text does not cover methods for pre-processing data because it requires a more detailed understanding of the experimental process that is described here. Fortunately, many of the preprocessing steps have been programmed in statistical languages such as \textsf{R} and are publicly available, and the pre-processed data from important experiments is often available from investigator web sites.  When analyzing `cleaned' data from an experiment such as a gene chip, it is important to be aware that inferences made about a biological condition depend to a very large extent on the nature of the experiment and the care that has been taken in preparing the data. 

In 1999, Golub and co-authors published one of the earliest successful uses of gene chip technology in a study of childhood leukemia\footnote{Golub, Todd R., et al. Molecular classification of cancer: class discovery and class prediction by gene expression monitoring. Science 286 (1999): 531-537.}.  The detailed results of the experiment have been superseded by newer methods in larger studies, but it is can viewed today as a proof of principle -- estimated differential expression levels of genes in a tumor can be used to classify the tumor into subtypes, supplementing what pathologists have done for years.  The idea has been used to develop clinical diagnostic tools in breast cancer (ref to papers) that are commercially available.  The Golub data set is not large by current standards, so it provides a useful setting to examine data from this class of experiments.

The Golub team used an Affymetrix gene chip to measure the expression levels of \textsl{mRNA} from 7129 genes using bone marrow or blood samples from 72 children with acute leukemia, 47 with acute lymphoblastic leukemia (ALL) and 25 with acute myeloblastic leukemia (AML). The goal of the experiment was to determine if particular genes were more likely to be expressed in ALL while other genes were more likely to be expressed in AML. In addition to recording leukemia type, the investigators also recorded a small number of clinical or demographic characteristics of the children. The original data (after some initial pre-processing) are available from Broad Institute\footnote{http://www-genome.wi.mit.edu/mpr/data\_set\_ALL\_AML.html}. The data used here have been preprocessed further and are discussed in Data Analysis and Graphics Using R: An Example-Based Approach By John Maindonald, W. John Braun. \textit{we should add this to the bibtex file for the book and ref by bib number}. In this version, the expression levels have been `normalized' so that the median and standard deviation of expression levels are the same  across the different chips. The measurements for each patient were made using separate chips, and this normalization adjusts for the chip to chip variability mentioned earlier.  The process is similar to, but more complicated than, the transformations discussed in Section~\ref{transformingDataSubsection}.
...
The expression data for the 7129 genes examined in the study is given in the last 7129 columns of the dataset.  Each of these columns is a variable with a name that corresponds to the name of the probe on the Affymetrix chip used in the study to measure the expression of a particular gene. For the time being, it is easiest to call these probe-names genes; the analysis of the data later in the text will include information on how to map the probe names into gene names and functions.

Table~\ref{sampleGolubData} shows 5 rows and 6 columns of data from the Golub experimental data.  Each row corresponds to a patient, with sample id given by the \var{Samples} variable  in the first column.  The second and third columns specify sex of the patient (\var{Gender} coded M or F for male and female) and cancer type (\var{cancer}, these 5 patients all had  B-cell ALL). The last three columns show values of normalized gene expression levels for  three genes.    The second row shows that patient with sample ID 40 had a higher expression at the probe \var{AFFX-BioB-3\_at} than at \var{AFFX-BioB-5\_at}. As mentioned earlier, for a gene chip analysis, the gene expression values are useful for comparing two or more genes.  Individual values do not have biological meaning.
...
Figure~\ref{golubSampleGenesBoxplot} shows a box plot of the normalized expression levels for three genes in table~\ref{sampleGolubData}.  The box plots show that the expression levels are approximately symmetrically distributed (after transformation).  Since the medians of these three expression levels are negative, these three genes tend to be under-expressed compared to some others in the study.  Useful visual displays in gene expression studies are more difficult than in the studies discussed earlier; side-by-side box-plots cannot be used with 7,129 genes.   

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.82\textwidth]
	{ch_intro_to_data_oi_biostat/figures/golubSampleGenesBoxplot/golubSampleGenesBoxplot.pdf}
	\caption{Expression levels for three genes.}
