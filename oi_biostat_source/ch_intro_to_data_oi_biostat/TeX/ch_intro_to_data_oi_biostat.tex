...
A \term{summary statistic} is a single number summarizing a large amount of data.\footnote{Formally, a summary statistic is a value computed from the data.} In the Peanut Avoidance group, the proportion of participants failing the food challenge at 5 years of age is 36/263 = 0.137 (13.7\%); in the Peanut Consumption group, the proportion failing is 5/267 = 0.019 (1.9\%). The difference between these two proportions, 11.8\%, is a summary statistic describing the extent to which these two proportions differ. A second summary statistic, the ratio of the two proportions (0.137/0.019 = 7.31), indicates that the proportion failing in the Avoidance group is more than 7 times that of the Consumption group. In other words, the risk of failing the food challenge was more than 7 times greater for participants in the Avoidance group than for those in the Consumption group; this statistic is referred to as the \term{relative risk}.  

\textit{JV: added a sentence here explicitly introducing relative risk. Should probably also find a spot for it later in the main text.}
	
The summary statistics for the LEAP study highlight an important point -- the results of a study can sometimes be surprising. A parent with a child already known to have existing food allergies might be justifiably skeptical about feeding peanut butter to their child. However, the LEAP study suggests that, at least for children similar to those enrolled in the study, the benefits of early exposure might be substantial. 
...
Researchers were interested in using the \data{famuss} data to answer several questions, including: is ACTN3 genotype associated with variation in muscle function? The ACTN3 gene codes for a protein involved in muscle function. A common mutation (polymorphism) at location 577 in the ACTN3 gene changes C to T; TT individuals are unable to produce any ACTN3 protein in their muscle. Thus, researchers hypothesized that ACTN3 genotype might influence muscle function in humans. The response variable in this study is \var{ndrm.ch}, the percent change in non-dominant arm strength, with strength gain being used as a way to measure muscle function. The explanatory variable of interest is \var{actn3.r557x}, ACTN3 genotype at location 577.  
...
The explanatory variable examined in the study is \var{altitude}. While \var{latitude} is an environmental factor that might potentially influence features of the egg clutches, it is not a variable of interest in this particular study.

The variables \var{egg.size}, \var{clutch.size}, and \var{clutch.volume} are response variables measuring the level of maternal investment.

Female body size (\var{body.size}) is neither an explanatory or response variable. 

\end{example}	

%JV: Should come back to this to point out that body.size is a potential confounder.
...
It is important to incorporate the three experimental design principles into a study whenever possible; this book describes applicable methods for analyzing data from such experiments. Blocking is a slightly more advanced technique, and statistical methods in this book may be extended to analyze data collected using blocking.
...
%\textit{The study in the next paragraph is too old to be interesting.  Let's replace it with something current.}
...
\textit{JV: Perhaps this would be a good spot to point out that confounding variables should also be accounted for in randomized studies, and maybe introduce an example that illustrates the concept. Alternatively, confounding variables could be introduced in the previous section and explained again here (use example of body.size with frog data). Important to not imply that confounding variables are only an issue with observational studies.}
...
Almost all statistical methods are based on the notion of implied randomness. If observational data are not collected from a population via a random framework, these statistical methods -- calculating estimates and errors associated with estimates -- are not reliable. Four random sampling techniques are discussed in the following section: simple, stratified, cluster, and multistage sampling. Figures~\ref{simple_stratified} and~\ref{cluster_multistage} provide graphical representations of these techniques.

\begin{figure}
\centering
\includegraphics[width=\textwidth]{ch_intro_to_data_oi_biostat/figures/samplingMethodsFigure/simple_stratified}
\caption{Examples of simple random\index{sample!simple random sampling} and stratified sampling\index{sample!stratified sampling}. In the top panel, simple random sampling was used to randomly select 18 cases (identified by the circled orange dots) out of the total population (blue dots). The bottom panel illustrates stratified sampling: cases were grouped into six strata, then simple random sampling was employed within \mbox{each stratum}.}
\label{simple_stratified}
\end{figure}

\termsub{Simple random sampling}{sample!simple random sampling} is the most intuitive form of random sampling. A sample is generally referred to as a "simple random sample" if each case in the population has an equal chance of being included in the sample \emph{and} if knowing that a certain case is included in a sample does not provide useful information about which other cases are included. For example, an integrated care provider might randomly sample names using its membership roster and contact those sampled with a telephone or mailed survey. 

\termsub{Stratified sampling}{sample!stratified sampling} is a divide-and-conquer sampling strategy. The population is divided into groups called \term{strata}\index{sample!strata|textbf}. The strata are chosen so that similar cases are grouped together; afterwards, a second sampling method, usually simple random sampling, is used to select cases within each stratum. Stratified sampling is especially useful when the cases in each stratum are very similar with respect to the outcome of interest, but cases between strata might be quite different. 

Recall that the health care provider is interested in learning how members perceive the quality of services offered. Suppose the health care provider has facilities in different cities. If the same services are not necessarily offered in all cities, but all locations in a given city will offer similar services, stratified sampling would be a reasonable choice of sampling method. Each city could represent a stratum, and plan members could be randomly sampled from each city.

\begin{example}{Why might it be helpful for cases within each stratum to be very similar?}
If cases within each stratum are very similar, it is possible to get a more precise estimate for the subpopulation in each stratum. These improved estimates for each subpopulation will increase the reliability of an estimate for the full population.
\end{example}

In a \termsub{cluster sample}{sample!cluster sample}, the population is first divided into many groups, called \termsub{clusters}{sample!cluster}. Then, a fixed number of clusters is sampled; all observations from each of those clusters are included in the sample. A \termsub{multistage sample}{sample!multistage sample} is similar to a cluster sample, but rather than keeping all observations in each cluster, a random sample is collected within each selected cluster.

\begin{figure}
\centering
\includegraphics[width=\textwidth]{ch_intro_to_data_oi_biostat/figures/samplingMethodsFigure/cluster_multistage}
\caption{Examples of cluster\index{sample!cluster sampling} and multistage sampling\index{sample!multistage sampling}. The top panel illustrates cluster sampling: data were binned into nine clusters, three of which were sampled, and all observations within these clusters were used as a sample. The bottom panel illustrates multistage sampling, which differs from cluster sampling in that only a subset from each of the three selected clusters are drawn to be included in the sample.}
\label{cluster_multistage}
\end{figure}

Unlike with stratified sampling, cluster and multistage sampling are most helpful when there is high case-to-case variability within a cluster, but the clusters themselves are similar to one another. For example, if neighborhoods in a city represent clusters, cluster and multistage sampling work best when the population within each neighborhood is very diverse.

Analysis of data collected using stratified, cluster, or multistage sampling is more complicated than for data from a simple random sample; the analysis methods discussed in this text would need to be extended to handle such data. Applying these sampling techniques can often be more economical than only drawing random samples.

\begin{example}{Suppose researchers are interested in estimating the malaria rate in a densely tropical portion of rural Indonesia. There are 30 villages in the area, each more or less similar to the next. The goal is to test 150 individuals for malaria. What sampling method should be employed?}
...
The \term{mean}, sometimes called the \indexthis{average}{mean!average}, is a common way to measure the center of a \term{distribution} of data. To find the average clutch volume for the observed egg clutches, add all the clutch volumes and divide by the total number of clutches. For computational convenience, the volumes are rounded to the first~decimal.
\begin{eqnarray}
...
Formulas and methods used to compute the variance and standard deviation for a population are similar to those used for a sample.\footnote{The only difference is that the population variance has a division by $n$ instead of $n-1$.}  Like the mean, the population values have special symbols: $\sigma_{}^2$\marginpar[\raggedright$\sigma_{}^2$\\\footnotesize population variance\\ \hspace{2mm}]{\raggedright$\sigma_{}^2$\\\footnotesize population variance\\ \hspace{2mm}} for the variance and $\sigma$\marginpar[\raggedright$\sigma$\\\footnotesize population standard deviation\\ \hspace{2mm}]{\raggedright$\sigma$\\\footnotesize population standard deviation\\ \hspace{2mm}} for the standard deviation. The symbol $\sigma$ \index{Greek!sigma@sigma ($\sigma$)} is the Greek letter \emph{sigma}.

%JV: I think it makes more sense for the following information to be introduced with histograms and the concept of a symmetric distribution.

\begin{comment}

\begin{tipBox}{\tipBoxTitle{standard deviation describes variability}
	The use of the standard deviation as a descriptor of variability is at least as important as the formulas. Usually 70\% of the data will be within one standard deviation of the mean and about 95\% will be within two standard deviations. This rule of thumb is not always accurate when there are a few observations with extremely large (or extremely small) values. }
\end{tipBox}
...
%DH: \textit{Something for Morgan: the subtleties of computing $Q_1$ and $Q_3$.}
...
\begin{table}[ht]
	...
	\begin{tabular}{l c cc c cc}
		...
		data without four largest observations && 831.8 & 493.92 && 867.9 & 349.2 \\
...
Graphs of distributions, such as histograms and boxplots, show important features that are not evident in numerical summaries.  These features may include asymmetry, outliers, peaks or clumps in a distribution where values are more likely to occur than at other values, or values that are impossible and have been measured in error.
...
\textit{We are missing something important here.  In the clutch data, the outliers reflect the right-skewing. That is different from one or two outliers that are special observartions in a symmetric data set.  Strictly speaking, observations in the tail of a skewed distribution are not outliers; they are a feature of the distribution type.  Have not re-written here to capture this, but we should.}
...
Since weight is associated with height, weight itself is not a good measure of whether someone is overweight. Body mass index (BMI) is a measure of weight that is less affected by a person's height. A BMI of 30 or above is considered obese.\footnote{\url{http://www.nhlbi.nih.gov/health/educational/lose_wt/risk.htm}} In the metric system, BMI is calculated as weight in kilograms (kg) divided by height squared ($\textrm {m}^{2}$). If height and weight are measured in inches and pounds, as in the \data{famuss} data, then BMI is weight in pounds (lb) divided by height squared ($\textrm {in}^{2}$), multiplied by 703. The \data{famuss} data includes the variable \var{bmi} for each participant, and Figure~\ref{famussHeightBmi} shows the relationship between \var{height} and \var{bmi}. The strong upward trend in Figure~\ref{famussHeightWeight} is no longer evident, indicating that \var{height} and \var{bmi} have a much weaker association. For this reason, health agencies such as the US National Institutes of Health (NIH) and the World Health Organization (WHO) use BMI as a measure of obesity. 
...
Scatterplots provide a visual display of the relationship between two variables.  \term{Correlation} is a numerical measure of the strength of a linear relationship.  It is usually denoted by $r$ and takes on values between -1 and 1. \marginpar[\raggedright$r$\\\footnotesize correlation coefficient]{\raggedright$r$\\\footnotesize correlation coefficient} The term \term{correlation coefficient} is also often used for $r$. If the paired values of two variables tend to lie on a line, then $r > 0$, the slope of the line will be positive, and the variables positively associated.
If $r < 0$, the line will have negative slope and the variables will be negatively associated.
...
When data are linearly related, correlation indicates the strength of that linear relationship. However, a large correlation coefficient does not necessarily guarantee the data are linearly related, since correlation can be calculated even when the data in a scatterplot do not tend to lie on a line. The correlation of the data shown in Figure~\ref{incomeLifeExpectancy} is $0.63$. In this case, the relatively large correlation suggests a strong relationship, but not one that is linear.  
...
Transformations other than the logarithm can be useful as well. For instance, the square root ($\sqrt{\text{original observation}}$) and inverse ($\frac{1}{\text{original observation}}$) are sometimes used.
...
%dh: I added a bib ref.  It is too technical for our population, so I will continue to hunt for something accessible.
...
%DH: The text will later include information about how to translate the probe names into gene names and functions.
...
%DH: Not sure if we should explain some of the scientific terms that are in this explanation...`reads', `transcriptome', `non-model organisms', `cDNA', `short-sequence reads'

%JV: cDNA and transcriptome are defined earlier in this section. I don't think reads and non-model organisms need to be explicitly defined.
