...
%\textit{Inserted on 20 May 2016. Check figures for relevance.  Think about adding the Belgium dental study here.}
...
\termsub{Simple random sampling}{sample!simple random sampling} is probably the most intuitive form of random sampling. An integrated care provider might randomly sample names using its membership roster and contact those sampled with a telephone or mailed survey.  In general, a sample is referred to as a ``simple random sample'' if each case in the population has an equal chance of being included in the sample \emph{and} knowing that a case is included in a sample does not provide useful information about which other cases are included.

\termsub{Stratified sampling}{sample!stratified sampling} is a divide-and-conquer sampling strategy. The population is divided into groups called \term{strata}\index{sample!strata|textbf}. The strata are chosen so that similar cases are grouped together, then a second sampling method, usually simple random sampling, is employed within each stratum.  If the health care provider has facilities in different cities, each city might represent a stratum, and plan members could be randomly sampled from each city.
...
% Yes to the above, but it will only be defensible if the sample of frogs in the dataset was random
...
\textit{Something for Morgan: the subtleties of computing $Q_1$ and $Q_3$.}
...
Any reason to leave this in as a comment?
...
%  Not sure if we should explain some of the scientific terms that are in this explanation...`reads', `transcriptome', `non-model organisms', `cDNA', `short-sequence reads'
