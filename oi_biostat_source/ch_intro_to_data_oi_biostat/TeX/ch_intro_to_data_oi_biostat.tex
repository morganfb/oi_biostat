...
Scatterplots provide a visual display of the relationship between two variables. It is usually denoted by $r$ and takes on values between -1 and 1. The term \term{correlation coefficient} is often used for $r$. If, in a scatterplot, the paired values of two variables tend to lie on a line, then if $r > 0$, the slope of the line will be positive and the variables are positively associated.
...
\textit{add a margin box for r}

The equation shows that $r$ is an average of products of terms, where the components are differences between values of$x$ or $y$ and their respective means, but re-scaled by standard deviations. 

Example~\ref{simpleCorExample} shows the calculation of correlation in a simple (albeit, artificial) dataset.


\begin{example}  Suppose the paired values of $x$ and $y$ are $(1,3), (2,4), and (3,0)$.  It is easy to show that $\overline{x} = 2$, $overline{y} = 3$, $sd(x) = 1$, and $sd(y) = 2.65$. 
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{crrrrrrcc}
            \hline
        $i$ &   $x_i$ & $y_i$ & $\overline{x}$ & $\overline{y}$ & $\text{sd}(x)$ & $\text{sd}(y)$  & $\left(\frac{x_i - \overline{x}}{\text{sd}(x)}\right)
	\left(\frac{y_i - \overline{y}}{\text{sd}(y)}\right)$ & $r$\\
            \hline
