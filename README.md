The following README is taken from OpenIntro (https://github.com/OpenIntroOrg) whose textbook and organization we have based the material in this repository on. Note that the folders that have oi_biostat after are chapters that we have edited from OpenIntro. The ones without are exactly the same as OpenIntro content. Below reproduced is the original README:
